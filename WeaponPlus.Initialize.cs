    ...
    public partial class WeaponPlus : TerrariaPlugin
    {
        ...
        private void OnGreetPlayer(GreetPlayerEventArgs args)
        {
            if (args != null)
            {
                ...
                if (config.进服时是否开启自动重读武器_WhetherToTurnOnAutomaticLoadWeaponsWhenEnteringTheGame)
                {
        ...
        {
            string tips1 = LangTipsGet("几乎所有的武器和弹药都能强化，但是强化结果会无效化词缀，作为补偿，前三次强化价格降低 80%") + "\n" +
            ...
            string tips2 = LangTipsGet("输入 /plus    查看当前该武器的等级状态和升至下一级需要多少材料") + "\n" +
                           LangTipsGet("输入 /plus load    将当前身上所有已升级的武器重新获取") + "\n" +
                           LangTipsGet("输入 /plus [damage/da/伤害] [up/down]    升级/降级当前武器的伤害等级") + "\n" +
                           LangTipsGet("输入 /plus [scale/sc/大小] [up/down]   升级/降级当前武器或射弹的体积等级 ±5%") + "\n" +
                           LangTipsGet("输入 /plus [knockback/kn/击退] [up/down]    升级/降级当前武器的击退等级 ±5%") + "\n" +
                           LangTipsGet("输入 /plus [usespeed/us/用速] [up/down]    升级/降级当前武器的使用速度等级") + "\n" +
                           LangTipsGet("输入 /plus [shootspeed/sh/飞速] [up/down]    升级/降级当前武器的射弹飞行速度等级，影响鞭类武器范围±5%") + "\n" +
            ...
            {
                args.Player.SendMessage($"{LangTipsGet("当前物品：")}{Lang.GetItemNameValue(select.id)}   {LangTipsGet("共计消耗：")}{cointostring(select.allCost, out List<Item> temp)}\n{select.ItemMess()}", getRandColor());
            }
            // load 和 clear
            else if (args.Parameters.Count == 1)
            {
                ...
                else if (args.Parameters[0].Equals("clear", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    else
                    {
                        ...
                        select = null;
                        args.Player.SendMessage(LangTipsGet("完全重置成功！钱币回收：") + sss, new Color(0, 255, 0));
                    }
                }
                else
            ...
            else if (args.Parameters.Count == 2)
            {
                ...
                if (args.Parameters[0].Equals("damage", StringComparison.OrdinalIgnoreCase) || args.Parameters[0].Equals("da", StringComparison.OrdinalIgnoreCase) || args.Parameters[0] == "伤害")
                {
                    if (update == 1)
                    {
                        ...
                        args.Player.SendMessage($"{Lang.GetItemNameValue(select.id)} {LangTipsGet("升级成功")}   {LangTipsGet("共计消耗：")}{cointostring(select.allCost, out List<Item> temp)}\n{select.ItemMess()}", getRandColor());
                    }
                    else if (update == -1)
                    {
                        if (select.damage_level == 0)
                            args.Player.SendInfoMessage(LangTipsGet("当前 0 级无需回置"));
                        else
                        {
                            //判断扣钱操作
                            //TODO
                            ...
                            DB.WriteDB(select);
                            ReplaceWeaponsInBackpack(args.Player.TPlayer, select, 0);
                            args.Player.SendMessage($"{Lang.GetItemNameValue(select.id)}  {LangTipsGet("降级成功")}   {LangTipsGet("共计消耗：")}{cointostring(select.allCost, out List<Item> temp)}\n{select.ItemMess()}", new Color(0, 255, 0));
                        }
                    }
                }
