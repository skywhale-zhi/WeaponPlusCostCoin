...
namespace WeaponPlus
{
    public class Config
    {
        ...
        public static Config LoadConfig()
        {
            if (!File.Exists(configPath))
            {
                Config config = new Config(
                    8, 6, 4, 8, 8, 2.0f, 3.0f, 100.0f, 2.5f, 3.0f, 1.0f, 0.2f, 50
                );
        ...
        public Config(int 近战武器升级攻速上限, int 魔法武器升级攻速上限, int 远程武器升级攻速上限, int 召唤武器升级攻速上限, int 其他武器升级攻速上限, float 武器升级伤害上限倍数, float 武器升级击退上限倍数, float 武器升级攻速上限倍数, float 武器升级尺寸上限倍数, float 武器升级射弹飞速上限倍数, float 花费参数, float 升级花费增加, int 最多升级次数)
        {
